   ...
   // var adMobBannerView = GADBannerView()
    ...
    func getUserDetails() {
        ...
        if userObj[USER_ABOUT_ME] != nil {
            self.aboutMeTxt.text = userObj[USER_ABOUT_ME] as? String == "" ? "Bu kullanıcı henüz bir biyografi oluşturmadı." : "\(userObj[USER_ABOUT_ME]!)"
            //aboutMeTxt.text = "\(userObj[USER_ABOUT_ME]!)"
        }
        else {
            aboutMeTxt.text = "Bu kullanıcı henüz bir biyografi oluşturmadı."
        }
        
        // Get joined since
        let date = Date()
        self.joinedLabel.text = "Katıldı: \(self.timeAgoSinceDate(userObj.createdAt!, currentDate: date, numericDates: true))"
        
        // Get verified
        if userObj[USER_EMAIL_VERIFIED] != nil {
            self.verifiedLabel.text = userObj[USER_EMAIL_VERIFIED] as? Bool == true ? "Onaylı Profil: Evet" : "Onaylı Profil: Hayır"
        } else {
        ...
        let imageFile = userObj[USER_AVATAR] as? PFFileObject
    ...
    @IBAction func optionsButt(_ sender: Any) {
        if PFUser.current() != nil {
            ...
            let blockUser = UIAlertAction(title: blockTitle, style: .default, handler: { (action) -> Void in
                // Block User
                if blockTitle == "Kullanıcı engelle" {
